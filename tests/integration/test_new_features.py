...
def test_run_validator_with_verdict_skips_gracefully(direct_vm, direct_deploy, direct_alice, direct_bob):
    """
    Document that strict_eq-based contracts (verdict) DO capture validators,
    ...
    contract = direct_deploy("contracts/verdict.py")
...
def test_web_render_text_mode_works(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    Used by verdict. Validates that mock_web intercepts WebRender calls.
    """
    direct_vm.mock_web(r".*example\.com.*", {"status": 200, "body": "No delivery was made."})
    direct_vm.mock_llm(r".*", '{"upheld": true}')
